/**
 * @Author: dongnan
 * @Description: 重写 UrlTemplateImageryProvider 省略前五级
 * @Date: 2022-05-20 23:27:27
 */
...
const pickFeaturesTags = Cesium.combine(tags, {
	i: iTag,
	j: jTag,
 ...
 * @property {Cesium.Rectangle} [rectangle=Cesium.Rectangle.MAX_VALUE] The rectangle, in radians, covered by the image.
 * @property {TilingScheme} [tilingScheme=Cesium.WebMercatorTilingScheme] The tiling scheme specifying how the ellipsoidal
 * surface is broken into tiles.  If this parameter is not provided, a {@link Cesium.WebMercatorTilingScheme}
 * is used.
 ...
 *    rectangle : Cesium.Cesium.Rectangle.fromDegrees(96.799393, -43.598214999057824, 153.63925700000001, -9.2159219997013)
 * });
...
Object.defineProperties(UrlTemplateImageryProvider.prototype, {
	 ...
	 * @type {TilingScheme}
	 * @readonly
	 * @default new Cesium.WebMercatorTilingScheme()
	 */
	tilingScheme: {
		get: function () {
			//>>includeStart('debug', pragmas.debug);
...
UrlTemplateImageryProvider.prototype.reinitialize = function (options) {
	...
	that._readyPromise = Promise.resolve(options).then(function (properties) {
		...
		const pickFeaturesResource = Cesium.Resource.createIfNeeded(properties.pickFeaturesUrl);

		that.enablePickFeatures = Cesium.defaultValue(properties.enablePickFeatures, that.enablePickFeatures);
		that._urlSchemeZeroPadding = Cesium.defaultValue(properties.urlSchemeZeroPadding, that.urlSchemeZeroPadding);
		...
		that._rectangle = Cesium.defaultValue(properties.rectangle, that._tilingScheme.rectangle);
		that._rectangle = Cesium.Rectangle.intersection(that._rectangle, that._tilingScheme.rectangle);
...
UrlTemplateImageryProvider.prototype.requestImage = function (x, y, level, request) {
	...
	//author:dongnan 这里就是跳过前面5级，根据自己需要设置
	if (level <= 5) {
		return;
	}
	//>>includeEnd('debug');
	return Cesium.ImageryProvider.loadImage(this, buildImageResource(this, x, y, level, request));
};
...
UrlTemplateImageryProvider.prototype.pickFeatures = function (x, y, level, longitude, latitude) {
	...
	if (!this.enablePickFeatures || !Cesium.defined(this._pickFeaturesResource) || this._getFeatureInfoFormats.length === 0) {
		return undefined;
	}
...
const degreesScratch = new Cesium.Rectangle();
let projectedScratchComputed = false;
const projectedScratch = new Cesium.Rectangle();
...
const ijScratch = new Cesium.Cartesian2();
...
function reverseZTag(imageryProvider, x, y, level) {
	...
	const reverseZ = Cesium.defined(maximumLevel) && level < maximumLevel ? maximumLevel - level - 1 : level;
...
function computeDegrees(imageryProvider, x, y, level) {
	...
	imageryProvider.tilingScheme.tileXYToCesium.Rectangle(x, y, level, degreesScratch);
	degreesScratch.west = Cesium.Math.toDegrees(degreesScratch.west);
	degreesScratch.south = Cesium.Math.toDegrees(degreesScratch.south);
	degreesScratch.east = Cesium.Math.toDegrees(degreesScratch.east);
	degreesScratch.north = Cesium.Math.toDegrees(degreesScratch.north);
...
function computeProjected(imageryProvider, x, y, level) {
	...
	imageryProvider.tilingScheme.tileXYToNativeCesium.Rectangle(x, y, level, projectedScratch);
...
function computeIJ(imageryProvider, x, y, level, longitude, latitude, format) {
	...
	const rectangle = imageryProvider.tilingScheme.tileXYToNativeCesium.Rectangle(x, y, level, rectangleScratch);
...
function computeLongitudeLatitudeProjected(imageryProvider, x, y, level, longitude, latitude, format) {
	...
	if (imageryProvider.tilingScheme.projection instanceof Cesium.GeographicProjection) {
		longitudeLatitudeProjectedScratch.x = Cesium.Math.toDegrees(longitude);
		longitudeLatitudeProjectedScratch.y = Cesium.Math.toDegrees(latitude);
	} else {
